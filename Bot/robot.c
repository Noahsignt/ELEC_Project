...
void setup_robot(struct Robot *robot){
    ...
    */
    ...
    robot->auto_mode = 0;
    robot->state = 0;
    robot->last_direction = robot->angle;
    robot->first = 0;

    printf("Press arrow keys to move manually, or enter to move automatically\n\n");
}
int robot_off_screen(struct Robot * robot){ //Checks if x too left or y too low
    if(robot->x < 0 || robot-> y < 0){
        return 0;
    }
    if(robot->x > OVERALL_WINDOW_WIDTH || robot->y > OVERALL_WINDOW_HEIGHT){ //Checks if x too right or y too high
        return 0;
    }
    return 1;
}
...
void robotAutoMotorMove(struct Robot * robot, int front_left_sensor, int front_right_sensor, int side_left_sensor, int side_right_sensor) {
    /*
        Start by rotating 90 degrees, if you sense a wall, move forward a bit, rotate back and hope side sensor picks it up.
    */

    if(robot->state == 0 && side_right_sensor != 0){
        robot->state = 2;
    }

    else if ((robot->currentSpeed>0) && ((front_left_sensor == 1) || (front_right_sensor == 1)) && robot->state == 0) {
        ...
        if(robot->angle < 45){
            robot->last_direction = 360;
        }
    }

     else if ((front_left_sensor == 0 && front_right_sensor == 0) && robot->state == 0){
        if(robot->first < 6){
        ...
        //printf("DIRECTION: %d", robot->angle);
    }

    else if (robot->state == 1) {

        if(robot->angle != robot->last_direction - 90){
        ...
        if(robot->first < 7){ //MUST CHANGE THIS DEPENDING ON MAZE
            robot->first += 1;
        }

        else if(side_right_sensor == 0){ //lol this is actually side right but anyway
            robot->last_direction = robot->angle;

            if(robot->angle == 270){
                robot->last_direction = -90;
            }
        ...
        if(robot->first < 3){ //terrible solution to bad sensor placement
            robot->first += 1;
        }

        else if(robot->first == 3){
            robot->first = 100;
            robot->direction = DOWN;
        }
        else if(robot->angle != robot->last_direction + 90){
            robot->direction = RIGHT;
